# Package metadata
TITLE         := PS4 Linux Launcher
VERSION       := 1.0
TITLE_ID      := LINU00003
CONTENT_ID    := IV0000-LINU00003_00-PS4LINUXLAUNCHER
...
CPPFILES      := $(filter-out $(SDIR)/CreditView.cpp,$(CPPFILES))
...
pkg.gp4: eboot.bin guest.elf sce_sys/param.sfo $(SCESYS) $(SCEMODULES) $(ASSETS)
	python3 scripts/create-gp4.py --out $@ --content-id=$(CONTENT_ID) --files "$^"
